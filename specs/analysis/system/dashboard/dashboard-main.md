...
**Data source**: `InfoService.getDashInfo(-1)` populates the entire dashboard via `$(document).trigger("reload")`. The response object shape is `data.*` with sub-collections.
