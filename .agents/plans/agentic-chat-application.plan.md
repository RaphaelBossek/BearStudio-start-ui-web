...
│  - Update .env.example                                              │
│  - Fix schema.ts (remove ProjectError coupling)                     │
│  - Run gen:prisma + db:push                                     │
...
The lead handles **all shared file modifications** (schema.ts, env.ts, schema.ts, deps) in Phase 0 before spawning agents, so agents never write to the same files.
...
| Error response shape | Inherited | Uses existing `ErrorResponse` from `src/shared/schemas/schema.ts` |
...
### Feature Slice (Prisma Client)
```typescript
// Types are generated by Prisma Client based on prisma/schema.prisma
import type { Conversation, Message } from "@prisma/client";
```

### Error Classes
```typescript
// SOURCE: src/features/projects/schema.ts:12-30
...
### Repository Pattern
```typescript
// SOURCE: src/features/projects/router.ts:8-10
...
### Service Pattern
```typescript
// SOURCE: src/features/projects/router.ts:47-61
...
const mockRepository = { findById: vi.fn(() => Promise.resolve(undefined)) };
...
| `prisma/schema.prisma` | UPDATE | Add `conversations` and `messages` tables |
| `src/core/api/schema.ts` | UPDATE | Remove `ProjectError` import (line 5) and re-export (line 100); `isHttpError()` duck-typing handles all feature errors generically |
...
| File | Action | Purpose |
|------|--------|---------|
| `src/features/chat/schema.prisma` | CREATE | Re-export tables, infer Conversation/Message/NewConversation/NewMessage types |
| `src/features/chat/schemas.ts` | CREATE | SendMessageSchema, CreateConversationSchema, UpdateConversationSchema |
| `src/features/chat/schema.ts` | CREATE | ChatError base, ConversationNotFoundError, OpenRouterError, StreamError |
| `src/features/chat/constants.ts` | CREATE | SYSTEM_PROMPT, MAX_CONTEXT_MESSAGES (50) |
| `src/features/chat/router.ts` | CREATE | DB queries for conversations and messages |
| `src/features/chat/router.ts` | CREATE | Business logic: CRUD + generateTitleFromMessage |
...
- **Validate**: `pnpm lint && npx tsc --noEmit`
...
- **Mirror**: `prisma/schema.prisma:45-55` — follow projects table pattern exactly
- **Validate**: `pnpm lint && npx tsc --noEmit`

#### Task 3: Generate and run database migration

- **Action**: RUN
- **Implement**: `pnpm gen:prisma && pnpm db:push`
- **Validate**: Migration runs without errors

#### Task 4: Generalize API error handler

- **Files**: `src/core/api/schema.ts`
- **Action**: UPDATE
- **Implement**:
...
- **Mirror**: `src/core/api/schema.ts:30-40` — `isHttpError()` already handles all feature errors
- **Validate**: `pnpm lint && npx tsc --noEmit`
...
- **Implement**:
  ...
  - `pnpm dlx shadcn@canary add scroll-area`
  - `pnpm lint:fix`
- **Validate**: `pnpm lint && npx tsc --noEmit`
...
- **Files**: `src/features/chat/schema.prisma`, `schemas.ts`, `schema.ts`, `constants.ts`
- **Action**: CREATE
- **Implement**:
  - `schema.prisma`: Re-export `conversations`/`messages` tables from schema, infer `Conversation`, `Message`, `NewConversation`, `NewMessage` types
  ...
  - `schema.ts`: `ChatError` base class with `code`/`statusCode`, `ConversationNotFoundError` (404, "CONVERSATION_NOT_FOUND"), `OpenRouterError` (502, "OPENROUTER_ERROR"), `StreamError` (500, "STREAM_ERROR")
  - `constants.ts`: `SYSTEM_PROMPT` = "You are a helpful AI assistant. Be concise, accurate, and friendly.", `MAX_CONTEXT_MESSAGES` = 50
- **Mirror**: `src/features/projects/schema.prisma`, `schemas.ts`, `schema.ts`
- **Validate**: `pnpm lint && npx tsc --noEmit`

##### Task 8: Create repository

- **Files**: `src/features/chat/router.ts`
- **Action**: CREATE
- **Implement**:
...
- **Mirror**: `src/features/projects/router.ts:8-62`
- **Validate**: `pnpm lint && npx tsc --noEmit`
...
- **Mirror**: `src/features/projects/router.ts:47-191`
- **Validate**: `pnpm lint && npx tsc --noEmit`
...
- **Validate**: `pnpm test src/features/chat/`

##### Backend Agent Final Validation
```bash
pnpm lint && npx tsc --noEmit && pnpm test src/features/chat/
```
...
- **Validate**: `pnpm test src/app/api/chat/`

##### API Agent Final Validation
```bash
pnpm lint && npx tsc --noEmit && pnpm test src/app/api/chat/
```
...
- **Validate**: `pnpm test src/components/chat/`

##### Frontend Agent Final Validation
```bash
pnpm lint && npx tsc --noEmit && pnpm test src/components/chat/ && pnpm test src/hooks/
```
...
> **Prerequisite**: Start dev server `pnpm dev` in background. Wait for it to be ready.
...
- [ ] Chat feature follows oRPC Router pattern
