...
|------|-----------|--------|-----------------|---------------------|
| topic | **Related to** | TOTP onboarding in login | [Admin TOTP Onboarding](_shared-components/totp-onboarding.md) | Login TOTP flow vs. admin TOTP device management |

> **Context:** This file's `login.js` documents the **login-time TOTP 2FA onboarding flow** (require.token → QR code → 6-digit verification). The linked [Admin TOTP Onboarding](_shared-components/totp-onboarding.md) file covers the **user settings TOTP device management UI** (register, verify, reset devices). Both share the same 6-digit split-input widget pattern and `UserService` backend calls.
