...
3. **Detail Inspection View**: Implementation of a Drawer showing categorized tabs (General, Timing, Participants, Location/Job, Counts, Raw Data) of the treatment entity.
...
1. **Prisma Schema**: Ensure `Treatment` model and all related types (`TreatmentUser`, `TreatmentJob`, `TreatmentCustomer`, `TreatmentLocation`, `TreatmentPosition`, etc.) are correctly mapped in `schema-mongodb.prisma`.
...
- [x] Detail view displays full object structure with categorized tabs.
