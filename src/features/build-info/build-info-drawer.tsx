...
export const BuildInfoDrawer = ({
  ...
  return (
    <ResponsiveDrawer>
      {children && <ResponsiveDrawerTrigger nativeButton={nativeButtonTrigger} render={children} />}
          ...
          <ResponsiveDrawerDescription className="flex min-h-6 items-center">
            {isCopied ? (
              <span className="flex items-center gap-1 rounded-md bg-positive-100 px-1.5 py-1 text-xs font-medium text-positive-800 max-sm:mx-auto dark:bg-positive-600/30 dark:text-positive-100">
                <CheckCircle2Icon className="size-3" /> {t('buildInfo:copiedToClipboard')}
              </span>
            ) : (
              <Button
                size="xs"
        ...
        <ResponsiveDrawerBody className="min-w-0 pb-8">
          <div className="flex flex-col">
            {Object.entries(buildInfo).map(([key, value]) => {
              ...
              return (
                <div key={key} className="flex border-b py-2 text-sm first:border-t">
                  <div className="w-24 font-medium text-muted-foreground capitalize">{key}</div>
                  <div className="min-w-0 flex-1 font-mono break-words">{value}</div>
                </div>
              );
            })}
          </div>
