...
### 1.2 Resource Path Specification
Resources are identified using path or URL-like syntax. This allows for granular control over various system components.
...
- **Precedence Rule**:
  - **Deepest Path First**: Permissions are applied starting from the most specific (deepest) path match. If no specific match is found, the system continues to evaluate increasingly general (parent) paths until a rule applies.
...
Users are assigned one of the following confidentiality levels, which defines their data clearance:
...
The confidentiality level assigned to a user determines how they perceive specific data attributes within a resource:

| Identifier | Technical Term | Description | Example (Credit Card) |
| :--- | :--- | :--- | :--- |
| `level_0_top_secret` | Clear Text | Full visibility for authorized users. | 4532 0122 8844 1092 |
| `level_1_secret` | Partial Masking | Shows "hints" for verification without full disclosure. | **** **** **** 1092 |
| `level_2_internal` | Pseudonymization | Replaces data with a reversible, realistic identifier. | 4912 8331 0021 7732 |
| `level_3_public` | Anonymization | Scrubbed or aggregated data; no individual identification. | Visa / North America |
| `level_4_restricted` | Redaction / Nulling | Total removal or nulling of the data attribute. | [REDACTED] or NULL |
