...
|------|-----------|-------|-----------------|----------|
| event | **Incoming** | `loadConsultation` | [Dashboard Main](../../system/dashboard/dashboard-main.md#block-active-appointments-card) | Triggered by `newPatient` / `startBasisWeb` button |
| event | **Outgoing** | `loadBasisweb` | [BasisWeb Wizard](../../interfaces/dashboard/basisweb-wizard.md#invocation) | `location.patientDataType == "EXTERNAL_BASISWEB"` && `appointment.state != "LOCKEDIN"` |
